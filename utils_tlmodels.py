...
#Customized Transfer Learning models
def GoogLeNet(train_imgs,train_labels,class_no,num_epochs=20):
  print("-------GOOGLENET--------")

  pre_trained_model1 = keras.applications.GoogLeNet(input_shape = (224,224,3),include_top = False,weights='imagenet')
  for layer in pre_trained_model1.layers:

    layer.trainable = False
  x = keras.layers.Flatten()(pre_trained_model1.output)
  x = layers.Dense(4096,activation='relu')(x)
  x = layers.Dropout(0.2)(x)
  x = layers.Dense(64,activation='relu')(x)
  x = layers.Dense(class_no,activation='softmax')(x)
  model1 = Model(pre_trained_model1.input,x)
  model1.compile(optimizer = Adam(learning_rate=3e-4),loss='categorical_crossentropy',metrics=['acc'])
  history = model1.fit(x=train_imgs,y=train_labels, epochs = num_epochs, batch_size = 32,verbose=0)
  print("----------------------------------")
  return model1
  
def MobileNetV2(train_imgs,train_labels,class_no,num_epochs=20):
  print("-------MobileNetV2--------")
  input_shape_pre_trained_model2 = (224,224,3)
  pre_trained_model2 = keras.applications.MobileNetV2(include_top=False,weights="imagenet",input_tensor=None,input_shape=input_shape_densenet,pooling=None)
  pre_trained_model2.trainable = True
  for layer in pre_trained_model2.layers:
    layer.trainable = False

  layer = keras.layers.Flatten()(pre_trained_model2.output)
  ...
  model2 = keras.models.Model(pre_trained_model2.input, outputs=layer)
  model2.compile(optimizer = keras.optimizers.Adam(learning_rate=3e-4),loss='categorical_crossentropy',metrics=['acc'])
...
def Xception(train_imgs,train_labels,class_no,num_epochs=20):
  ...
  model3.compile(optimizer = Adam(learning_rate=3e-4),loss='categorical_crossentropy',metrics=['acc'])
